...
//        [vButton setBackgroundImage:[UIImage imageNamed:vHeligtImageStr] forState:UIControlStateSelected];
        ...
        [vButton setTitleColor:[UIColor lightGrayColor] forState:UIControlStateNormal];
        [vButton setTitleColor:[UIColor colorWithRed:202/255.0 green:199/255.0 blue:136/255.0 alpha:1] forState:UIControlStateSelected];
        [vButton setTag:i];
        [vButton addTarget:self action:@selector(menuButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
        [vButton setFrame:CGRectMake(menuWidth-40, 0, vButtonWidth, self.frame.size.height)];
...
-(void)moveScrolViewWithIndex:(NSInteger)aIndex{
    ...
    if (vButtonOrigin >= 300) {
        if ((vButtonOrigin + 180) >= mScrollView.contentSize.width) {
            [mScrollView setContentOffset:CGPointMake(mScrollView.contentSize.width - 260, mScrollView.contentOffset.y) animated:YES];
            return;
        }
...
//        NSLog(@"scrollwOffset.x:%f,ButtonOrigin.x:%f,mscrollwContentSize.width:%f",mScrollView.contentOffset.x,vButtonOrigin,mScrollView.contentSize.width);
    }else if (vButtonOrigin<=120)
    {
        [mScrollView setContentOffset:CGPointMake(-140, mScrollView.contentOffset.y) animated:YES];
        return;
    }else{
